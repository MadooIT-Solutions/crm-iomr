...
class CRMLead(models.Model):
    ...
    rotation_history_ids = fields.One2many(
        "crm.lead.rotation", "lead_id", string="Histórico de Rotação"
    )

    last_activity_date = fields.Datetime(
        string="Última Atividade", compute="_compute_last_activity_date", store=True
    )
    ...
    def _compute_last_activity_date(self):
        for lead in self:
            ...
            lead.last_activity_date = (
                activities[0].date_deadline if activities else None
            )
        ...
        for lead in self:
            if lead.days_with_current_seller >= daystochange:
                ...
                # Se passou total_days, ir para SDR
                    ...
                    lead.days_with_current_seller >= total_days
                    and lost_sdr_enabled
                    and sdr_user_id
                ):
                    lead.write(
                        {
