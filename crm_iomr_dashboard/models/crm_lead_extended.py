...
class CrmLead(models.Model):
    ...
    weighted_forecast = fields.Monetary(
        string="Forecast Ponderado",
        currency_field="company_currency",
        compute="_compute_weighted_forecast",
        store=True,
    )
    acquisition_cost = fields.Monetary(
        string="Custo de Aquisição", currency_field="company_currency"
    )
    ...
    surgery_value = fields.Monetary(
        string="Valor da Cirurgia", currency_field="company_currency"
    )
    ...
    def _compute_roi(self):
        for lead in self:
            if lead.acquisition_cost > 0:
                lead.roi_value = (
                    (lead.expected_revenue - lead.acquisition_cost)
                    / lead.acquisition_cost
                ) * 100
            else:
                lead.roi_value = 0.0
