...
class CrmIomrSnapshot(models.Model):
    ...
    total_weighted_forecast = fields.Monetary(
        string="Total Forecast Ponderado", currency_field="currency_id"
    )
    avg_ticket = fields.Monetary(string="Ticket Médio", currency_field="currency_id")
    avg_roi = fields.Float(string="ROI Médio (%)")
    total_surgeries_value = fields.Monetary(
        string="Valor Total Cirurgias", currency_field="currency_id"
    )
    surgery_forecast_count = fields.Integer(
        string="Oportunidades em Marcação Cirúrgica"
    )

    # Recovery
    recovered_revenue = fields.Monetary(
        string="Receita Recuperada", currency_field="currency_id"
    )
    ...
    transferred_closure_rate = fields.Float(
        string="Taxa de Fechamento pós-Transferência (%)"
    )
    ...
    def _get_surgery_stage(self):
        return self.env.ref(
            "crm_iomr_dashboard.stage_marcacao_cirurgica", raise_if_not_found=False
        )
    ...
    def action_generate_weekly_snapshot(self):
        ...
        user_leads = {
            u.id: self.env["crm.lead"].search([("user_id", "=", u.id)])
            for u in all_users
        }
        resp_times = []
        conv_rates = []
        ...
        transferred = self.env["crm.lead"].search(
            [("transferred_from_id", "!=", False)]
        )
        transferred_count = len(transferred)
        closed_after_transfer = transferred.filtered(lambda l: l.probability == 100)
        transfer_closure = (
            (len(closed_after_transfer) / transferred_count * 100)
